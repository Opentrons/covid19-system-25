...
def run(protocol):
    ...
    if PIP_CTRL:
        ...
        n2c = [well for col in ctrl_blk.columns()[2:4] for well in col]
        n3c = [well for col in ctrl_blk.columns()[4:6] for well in col]

        for src, dest in zip([n1, n2, n3], [n1c, n2c, n3c]):
            pick_up(s300)
            for well in dest:
    ...
    src_tubes = ctrl_blk.rows()[0][:6]
    dest_wells = [
        [plate384.rows()[0][:12:3], plate384.rows()[0][12::3]],
        [plate384.rows()[1][:12:3], plate384.rows()[1][12::3]],
        [plate384.rows()[0][1:13:3], plate384.rows()[0][13::3]],
        [plate384.rows()[1][1:13:3], plate384.rows()[1][13::3]],
        [plate384.rows()[0][2:14:3], plate384.rows()[0][14::3]],
        [plate384.rows()[1][2:14:3], plate384.rows()[1][14::3]]
        ]

    m20vol = 0
    for src, destrows in zip(src_tubes, dest_wells):
        pick_up(m20)
        for dest in destrows:
            for well in dest:
                if m20vol < 4:
                    m20vol = round(m20vol, 1)
